...
class Create_Session extends StatelessWidget {
            ...
            TextButton.icon(
              ...
              onPressed: () async {
                if(_formKey.currentState.validate()){
                    ...
                    await DatabaseService().updateSessionData(Listtitle, sets);
                }
              },
            ),
          ],
        ...
        Padding(
          ...
          child: Row(
            children: [
              ...
              SizedBox(width: 16,),
              Expanded(child: RepsFields(i)),
              SizedBox(width: 16,),
              // we need add button at last friends row
              _addRemoveButton(i == sets.length-1, i),
            ],
          ),
        )
      );
    ...
    return InkWell(
      onTap: (){
        if(add){
          ...
          sets.insert(sets.length, null);
        }
        else sets.removeAt(index);
        setState((){});
      },
      child: Container(
        width: 30,
...
class _TrickTextFieldsState extends State<TrickTextFields> {
  ...
  Widget build(BuildContext context) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      _nameController.text = _TrickListFormState.sets[widget.index].trick ?? '';
    });

    return TextFormField(
      controller: _nameController,
      onChanged: (val) => _TrickListFormState.sets[widget.index].trick = val,
...
// TrickTextField
class RepsFields extends StatefulWidget {

  final int index;

  RepsFields(this.index);

  @override
  _RepsFieldsState createState() => _RepsFieldsState();
}

class _RepsFieldsState extends State<RepsFields> {
  TextEditingController _nameController;
  ...
  Widget build(BuildContext context) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      _nameController.text = _TrickListFormState.sets[widget.index].reps ?? '';
    });

    return TextFormField(
      controller: _nameController,
      onChanged: (val) => _TrickListFormState.sets[widget.index].reps = int.parse(val),
      style: TextStyle(color: Colors.grey[100], fontSize: 16.0),
      decoration: InputDecoration(
        hintText: 'Trick name',
        hintStyle: TextStyle( 
